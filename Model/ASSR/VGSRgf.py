...
import sys
import os
from Model.ASSR.VGT import UGT
...
class DAE(nn.Module):
    def __init__(self, in_channels, h, out_channels):
        super().__init__()
        # self.ridgelet = UGT(in_channels=in_channels,num_scales=3,max_directions=h,wavelet_scales=2)
...
class VDAF(nn.Module):
    ...
    def forward(self, x, y):
        x = F.interpolate(x, scale_factor=2, mode='bicubic', align_corners=False)  # 八倍
...
class VGSR_NET(nn.Module):
    def __init__(self, hsi_channels, msi_channels, h):
        ...
        self.msi_projection = FeatureTransformBlock(150, 4)
        self.GSR = Block(150, 150, dim=96,ffn_expansion_factor=2.66,avg_nums=3, num=2)
