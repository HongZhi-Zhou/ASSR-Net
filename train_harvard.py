...
# from Models.RIDGE_NET_v5 import SSHNET, BiSER
...
if __name__ == '__main__':
    ...
    data_name='Harvard'
    path1 = r'Dataset/Harvard/train/'
    path2 = r'Dataset/Harvard/test/'
    # 训练参数
    ...
    last_epoch = -1
    if resume == 1:
        model_path = r"C:\Users\keff\Desktop\RIDGE_NET\PKL\Cave\BiSERv1\4\cave_pkl\current.pkl"
        checkpoint = torch.load(model_path)
        cnn.load_state_dict(checkpoint)
    optimizer = torch.optim.Adam(cnn.parameters(), lr=LR,betas=(0.9, 0.999),weight_decay=weight_decay)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, maxiteration, eta_min=4e-5, last_epoch=last_epoch)
    start_epoch = 0
    for epoch in range(start_epoch+1, EPOCH+1):
        ...
        for a1, a2, a3 in loop:
            ...
            output,_1 = cnn(a2.cuda(),a3.cuda(),True)
            loss1 = loss_func(output,a1.cuda())
            loss2 = loss_func(_1,a1.cuda())
            loss = loss1*0.7+loss2*0.3
            # loss,recon,proj= loss_func(output, a1.cuda(),_1,a3.cuda(),_2)
            loss_temp = loss
            # loss_proj.append(np.array(proj.detach().cpu().numpy()))
            # loss_fuse.append(np.array(recon.detach().cpu().numpy()))
            loss_all.append(np.array(loss_temp.detach().cpu().numpy()))
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            loop.set_description(f'Epoch [{epoch}/{EPOCH}]')
            loop.set_postfix({'all': '{0:1.8f}'.format(np.mean(loss_all)),"lr": '{0:1.8f}'.format(lr)})
            scheduler.step()
        if epoch%checkpoint_interval == 0:
            torch.save(cnn.state_dict(), pkl_path + '/' + 'current.pkl') # 添加断点
        if ((epoch % val_interval == 0) and (epoch >= val_epoch)) or epoch==EPOCH:
            torch.save(cnn.state_dict(), pkl_path + '/' +str(epoch)+ '.pkl') # 添加断点
            cnn.eval()
